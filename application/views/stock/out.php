<div class="main-content">
  <div class="page-header d-flex justify-content-between align-items-center">
    <div>
      <h1>Stok Keluar</h1>
      <p>Input pengurangan stok obat.</p>
    </div>
    <div>
  ...
  <div class="card">
    <div class="card-body">
      <form method="post" action="<?= base_url('stock/out') ?>">
          ...
          <div class="col-md-6">
            ...
            <select name="medicine_id" class="form-select" required>
              ...
              <?php foreach ($medicines as $m): ?>
                <option value="<?= (int)$m['id'] ?>"><?= htmlspecialchars($m['name']) ?> (<?= htmlspecialchars($m['code']) ?>) — Stok: <?= (int)$m['current_stock'] ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-3">
        ...
        <div class="mt-4">
          <button class="btn btn-danger"><i class="bi bi-box-arrow-up"></i> Simpan Stok Keluar</button>
        </div>
      </form>
    </div>
  </div>
